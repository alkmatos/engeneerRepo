...
# Área..............................: TI HOLDING
#
# Cria tabelas na camada SILVER a partir da BRONZE, padroniza as tabelas 
# e atributos conforme THESAURUS, remove as mascaras dos CPF, CNPJ e CEP, 
# e elimina os espaçamentos laterais de todas colunas que são string
# colocando tudo que for string como UPPER.
# Ao final faz alguns tratamentos para os campos de CPF, CNPJ e CEP
# padronizando conforme solicitado na documentação do projeto e
# anonimizando os dados utilizando a function do databricks
#
# IMPORTANTE: Este atendimento atenderá todas as demandas nao havendo
# necessidades de novos desenvolvimentos, apenas adequações dependendo
# da necessidade devido alguma particularidade.
# Deve-se apenas alterar os parâmetros contidos nos CMD2 e CMD3
#******************************************************************************

# COMMAND ----------

from pyspark.sql.functions import col, lit, concat, lit, length, trim, when, udf
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    dicionario={'schema':SILVER_SCHEMA,'tabela':tb,'status':'ERRO','observacao':e,'dataExec':dt,'hora':hr}
                    ...
                    if colIsString != '[]':
                        for isStr in colIsString.split(","):
                            ...
                            sqlCM1=""
                            #* Monta os comandos
                            sqlCM0="update {}.{} set ".format(SILVER_SCHEMA,tb)+isStr+"=trim("+isStr+");"
                            sqlCM1="update {}.{} set ".format(SILVER_SCHEMA,tb)+isStr+"=upper("+isStr+");"
                            spark.sql(sqlCM0)     
                            spark.sql(sqlCM1) 
                    #* Remove as mascaras de CNPJ, CPF e CEP
                    if colTransforma != '[]':
                        for colunaT in colTransforma.split(","):
                            tamanhoPadronizacao=int(colunaT.split("-")[1].replace("'",'').replace("[",'').replace("]",''))
                            colunaT=colunaT.split("-")[0]
                            ...
                            for strChar in listaCaracteres:
                                ...
                                sqlCM2="update {}.{} set ".format(SILVER_SCHEMA,tb)+colunaT+"= replace("+colunaT+",'{}'".format(strChar)+",'');"
                                #* executa o comando
                                spark.sql(sqlCM2)
                            #* Tratamento para CEP, CNPJ e CEP adiconando zeros a esquerda
                            #  quando necessário
                            spkDf1=""
                            comando0=""
                            comando1=""
                            ...
                            comando5=""
                            comando6=""
                            #* Tratamento de CEP, adicionando zeros a esquerda quando
                            #  o tamnho do CEP for menor que 8
                            if tamanhoPadronizacao == 8:
                                spkDf1=spark.sql("select * from {}.{}".format(SILVER_SCHEMA,tb))
                                #* Garante que o tipo da coluta é STRING
                                comando1="spkDf1=spkDf1.withColumn("+ "'{}'".format(colunaT)+",spkDf1."+'{}'.format(colunaT)+".cast('String'))"
                                exec(comando1)
                                #* Preenche valores nulos como padrao '00000000'
                                comando2="spkDf1=spkDf1.na.fill(value='00000000',subset=['"+"{}".format(colunaT)+"'])"
                                exec(comando2)
                                #* Comandos contendo conditions para checar o tamanho do campo
                                #  e tratar
                                comando3='spkDf1=spkDf1.withColumn("'+'{}'.format(colunaT)+'", f.when(length(spkDf1.'+'{}'.format(colunaT)+') == 1,concat(lit("0000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando3=comando3+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 2,concat(lit("000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando3=comando3+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 3,concat(lit("00000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando3=comando3+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 4,concat(lit("0000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando3=comando3+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 5,concat(lit("000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando3=comando3+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 6,concat(lit("00"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando3=comando3+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 7,concat(lit("0"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando3=comando3+'.otherwise(spkDf1.'+'{}'.format(colunaT)+'))'
                                exec(comando3)
                                #* Cria tabela temporaria do dataframe para subir como tabela final
                                spkDf1.createOrReplaceTempView("tabTemporaria")
                                #* Sobe a tabela tratada
                                criaTabela="create or replace table {}.{} \n AS SELECT * from {}\n".format(SILVER_SCHEMA,tb,"tabTemporaria")
                                spark.sql(criaTabela)
                            #* CPF/CNPJ NO MESMO ATRIBUTO
                            # Como CCPF_CNPJ_SEGRD estao no mesmo atributo ficou definida a regra:
                            # - Se tamnaho do do campo < 11 tratar como CPF
                            # - Se tamnaho do do campo > 11 tratar como CNPJ
                            if tamanhoPadronizacao == 0:
                                spkDf1=spark.sql("select * from {}.{}".format(SILVER_SCHEMA,tb))
                                comando1="spkDf1=spkDf1.withColumn("+ "'{}'".format(colunaT)+",spkDf1."+'{}'.format(colunaT)+".cast('String'))"
                                exec(comando1)
                                comando0="spkDf1=spkDf1.na.fill(value='00000000000',subset=['"+"{}".format(colunaT)+"'])"
                                exec(comando0)
                                comando4='spkDf1=spkDf1.withColumn("'+'{}'.format(colunaT)+'",f.when((length(spkDf1.'+'{}'.format(colunaT)+') < 11) & (length(spkDf1.'+'{}'.format(colunaT)+') == 1),concat(lit("0000000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando4=comando4+'.when((length(spkDf1.'+'{}'.format(colunaT)+') < 11) & (length(spkDf1.'+'{}'.format(colunaT)+') == 2),concat(lit("000000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando4=comando4+'.when((length(spkDf1.'+'{}'.format(colunaT)+') < 11) & (length(spkDf1.'+'{}'.format(colunaT)+') == 3),concat(lit("00000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando4=comando4+'.when((length(spkDf1.'+'{}'.format(colunaT)+') < 11) & (length(spkDf1.'+'{}'.format(colunaT)+') == 4),concat(lit("0000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando4=comando4+'.when((length(spkDf1.'+'{}'.format(colunaT)+') < 11) & (length(spkDf1.'+'{}'.format(colunaT)+') == 5),concat(lit("000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando4=comando4+'.when((length(spkDf1.'+'{}'.format(colunaT)+') < 11) & (length(spkDf1.'+'{}'.format(colunaT)+') == 6),concat(lit("00000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando4=comando4+'.when((length(spkDf1.'+'{}'.format(colunaT)+') < 11) & (length(spkDf1.'+'{}'.format(colunaT)+') == 7),concat(lit("0000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando4=comando4+'.when((length(spkDf1.'+'{}'.format(colunaT)+') < 11) & (length(spkDf1.'+'{}'.format(colunaT)+') == 8),concat(lit("000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando4=comando4+'.when((length(spkDf1.'+'{}'.format(colunaT)+') < 11) & (length(spkDf1.'+'{}'.format(colunaT)+') == 9),concat(lit("00"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando4=comando4+'.when((length(spkDf1.'+'{}'.format(colunaT)+') < 11) & (length(spkDf1.'+'{}'.format(colunaT)+') == 10),concat(lit("0"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando4=comando4+'.when((length(spkDf1.'+'{}'.format(colunaT)+') > 11) & (length(spkDf1.'+'{}'.format(colunaT)+') == 12),concat(lit("00"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando4=comando4+'.when((length(spkDf1.'+'{}'.format(colunaT)+') > 11) & (length(spkDf1.'+'{}'.format(colunaT)+') == 13),concat(lit("0"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando4=comando4+'.otherwise(spkDf1.'+'{}'.format(colunaT)+'))'
                                exec(comando4)
                                spkDf1.createOrReplaceTempView("tabTemporaria2")
                                criaTabela2="create or replace table {}.{} \n AS SELECT * from {}\n".format(SILVER_SCHEMA,tb,"tabTemporaria2")
                                spark.sql(criaTabela2)
                            #* Trata / padroniza CPF tamanho 11
                            if tamanhoPadronizacao == 11:
                                spkDf1=spark.sql("select * from {}.{}".format(SILVER_SCHEMA,tb))
                                comando1="spkDf1=spkDf1.withColumn("+ "'{}'".format(colunaT)+",spkDf1."+'{}'.format(colunaT)+".cast('String'))"
                                exec(comando1)
                                comando0="spkDf1=spkDf1.na.fill(value='00000000000',subset=['"+"{}".format(colunaT)+"'])"
                                exec(comando0)
                                comando5='spkDf1=spkDf1.withColumn("'+'{}'.format(colunaT)+'",f.when(length(spkDf1.'+'{}'.format(colunaT)+') == 1,concat(lit("0000000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando5=comando5+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 2,concat(lit("000000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando5=comando5+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 3,concat(lit("00000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando5=comando5+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 4,concat(lit("0000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando5=comando5+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 5,concat(lit("000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando5=comando5+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 6,concat(lit("00000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando5=comando5+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 7,concat(lit("0000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando5=comando5+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 8,concat(lit("000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando5=comando5+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 9,concat(lit("00"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando5=comando5+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 10,concat(lit("0"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando5=comando5+'.otherwise(spkDf1.'+'{}'.format(colunaT)+'))'
                                exec(comando5)
                                spkDf1.createOrReplaceTempView("tabTemporaria3")
                                criaTabela3="create or replace table {}.{} \n AS SELECT * from {}\n".format(SILVER_SCHEMA,tb,"tabTemporaria3")
                                spark.sql(criaTabela3)
                            #* Trata / padroniza CNPJ tamanho 14
                            if tamanhoPadronizacao == 14:
                                spkDf1=spark.sql("select * from {}.{}".format(SILVER_SCHEMA,tb))
                                comando1="spkDf1=spkDf1.withColumn("+ "'{}'".format(colunaT)+",spkDf1."+'{}'.format(colunaT)+".cast('String'))"
                                exec(comando1)
                                comando0="spkDf1=spkDf1.na.fill(value='00000000000',subset=['"+"{}".format(colunaT)+"'])"
                                exec(comando0)
                                comando6='spkDf1=spkDf1.withColumn("'+'{}'.format(colunaT)+'",f.when(length(spkDf1.'+'{}'.format(colunaT)+') == 1,concat(lit("0000000000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando6=comando6+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 2,concat(lit("000000000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando6=comando6+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 3,concat(lit("00000000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando6=comando6+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 4,concat(lit("0000000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando6=comando6+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 5,concat(lit("000000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando6=comando6+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 6,concat(lit("00000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando6=comando6+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 7,concat(lit("0000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando6=comando6+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 8,concat(lit("000000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando6=comando6+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 9,concat(lit("00000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando6=comando6+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 10,concat(lit("0000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando6=comando6+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 11,concat(lit("000"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando6=comando6+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 12,concat(lit("00"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando6=comando6+'.when(length(spkDf1.'+'{}'.format(colunaT)+') == 13,concat(lit("0"),spkDf1.'+'{}'.format(colunaT)+'))'
                                comando6=comando6+'.otherwise(spkDf1.'+'{}'.format(colunaT)+'))'
                                exec(comando6)
                                spkDf1.createOrReplaceTempView("tabTemporaria4")
                                criaTabela4="create or replace table {}.{} \n AS SELECT * from {}\n".format(SILVER_SCHEMA,tb,"tabTemporaria4")
                                spark.sql(criaTabela4)
                    #* Seta data e hora para incluir no log de execução
                    dt = datetime.now().strftime("%Y-%m-%d")
                    hr = datetime.now().strftime("%H:%M:%S")
                    #* Monta informações de log que finalizaram com sucesso
                    dicionario={'schema':SILVER_SCHEMA,'tabela':tb,'status':'SUCESSO','observacao':'Sucesso','dataExec':dt,'hora':hr}
                    #* Adiciona o log no dataframe para depois apendar/concatenar no dataframe de log
                    dfPdLogTemp=pd.DataFrame([dicionario])
                    #* Inclui/concatena informações no log
                    dfPdLog = pd.concat([dfPdLog, dfPdLogTemp],ignore_index=True)
...
#** Anonimização das tabelas utilizando function do databricks
...
for tb in listaSchemaDelta:
    ...
    try:
        ...
        try:
            ...
            try:
                testTable=spark.sql("SELECT * FROM {} LIMIT 1;".format(tbT))
            except:
                print("TABELA [ {} ] NAO EXISTE.".format(tbT))
            else:
                criaView=""
                contaVirgula=strColunas.count(",")
                contaVirgula+=1
                columnaTabela=strColunas.replace("[","").replace("]","")
                colunaPThesaurus=colunaPadraoT.replace("[","").replace("]","")
                #* Colunas que devem ser anonimizadas
                if strRash != '[]':
                    print("ANONIMIZANDO A TABELA {}".format(tbT))
                    print("ATRIBUTOS {}".format(strRash))
                    tempRash=strRash
                    tempRash=tempRash.replace("[","").replace("]","").replace("'","")
                    comandoSql1="alter table {}".format(tbT)
                    #* Pega as colunas das tabelas que possuem dados para anonimização
                    #  monta a query e anonimiza
                    for coluna in tempRash.split(','):
                        comandoSql2=""
                        comandoSql2=comandoSql1+"\nalter column {} set mask udf_mask;".format(coluna.strip())
                        spark.sql(comandoSql2)
